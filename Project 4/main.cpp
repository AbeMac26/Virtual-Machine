...
//  Project4
//
//  Created by Abel Macias on 4/7/21.
...
string instType(string op) {
    ...
    if ("BLK" == op) {
        return to_string(BLK);
    }
    if ("LCK" == op) {
        return to_string(LCK);
    }
    if ("ULK" == op) {
        return to_string(ULK);
    }
    if ("TRP" == op) {
    ...
    if (file.is_open()) {
        while (file.good()) {
            ...
            /* Second pass: Will store instrcution to memory*/
                ...
                if(out.size()==1)
                    if (isInstruction(out.at(0)))
                        count += storeToMem(out.at(0),out.at(0),count) + 8;
            }
            if (file.peek() == EOF) {
                break;
            }
...
bool locked(int x){
	if (mutexLock.find(readInt(x)) != mutexLock.end())
         if (mutexLock[readInt(x)] != threads.front())
         	return true;
...
int main(int argc, const char* argv[]) {
    ...
    int thread_size = 4000;
    int thread_ID =1;
    reg[PC] = -1;
    reg[SB] = stack;
    reg[FP] = 0;
    reg[SP] = stack;
    int mainT = stack-(1024*16);
    threads.push(mainT);
    bool running = true;
    ...
    while (running) {
        ...
        switch (IR) {
            ...
            case CMP:
                ...
                break;
            case RUN:
                nextTaddress = mainT - thread_size *(thread_ID);
                reg[readInt(reg[PC] + op1)] = thread_ID++;
                threads.push(nextTaddress);
                for (int i = 0; i < 8; i++)
                writeInt(to_string(reg[i]), (nextTaddress-(4*i)));
                
                writeInt(to_string(readInt(reg[PC]+op2)), (nextTaddress-(4*(PC))));
                writeInt(to_string(mainT - thread_size*(thread_ID)), (nextTaddress-(4*(SL))));
                writeInt(to_string(nextTaddress-(4*(13))), (nextTaddress-(4*(SP))));
                writeInt(to_string(nextTaddress-(4*(13))), (nextTaddress-(4*(FP))));
                writeInt(to_string(nextTaddress-(4*(13))), (nextTaddress-(4*(SB))));
                reg[PC] += 12;
                break;
            case END:
                if (threads.front() == mainT || threads.size() == 1){
                    reg[PC] +=12;
                    break;
                }
                threads.pop();
            ...
            case BLK:
                ...
                break;
            case LCK:
                 if (locked(reg[PC] + op1))
                     break;
                mutexLock[readInt(reg[PC]+op1)] = threads.front();
                reg[PC] += 12;
                break;
            case ULK:
                mutexLock.erase(mutexLock.find(readInt(reg[PC]+op1)));
                reg[PC] += 12;
                break;
            case TRP:
       ...
       if(!contextSwitch){
        ...
        nextTaddress = threads.front();
        for (int i=0; i<13; i++)
           reg[i] = readInt(nextTaddress-(4*i));
    }
    return 0;
}
