...
while       TRP 2		# Read input
        	STR R3 _n	# store value to _n
    		LDR R0 ZERO  
    		STR R0 T5
        	CMP R0 R3
        	BRZ R0 prtArray
    ...
    # Create Activation record  and Invoce function
			...
			ADI  R6  120       # Compute return address
			...
			LDR R0 T5        # test Return value
        	 
    # _______Print________
        	LDB R3 F
        	TRP 3
        	LDB R3 i
        	TRP 3
...
#______________________fib(int n)____________________________________
fib_I 		MOV R7 SP           # Test for overflow SP < SL
           	ADI R7 -32          # Adjust for space needed (Rtn address & PFP)
            CMP R7 SL
           	BLT R7 OVERFLOW

	# if (n <= 1)
if_I		MOV R7 FP    		# Save FP in R7 this is PFP
	...
	#return n
        	...
        	LDR R7 (R7)
			STR R0 (SP)	
			LDR R1 T5        # load address of T5 to R1
       	 	ADD R1 R0        # add return value to value of T5
        	STR R1 T5        # stores return value to variable T5	 
        	JMR R7


ELSE_I  	LDR R0 (R7)  # load n
			MOV R1 R0
	# Calculate T1 = n-1
			ADI R0 -1
			ADI R7 -4
	...
	#____________________fib(T1)________________________
			MOV  R7  SP        # Test for overflow SP < SL
			ADI  R7  -32       # Adjust for space needed (Rtn address & PFP)
			CMP  R7  SL
			BLT  R7  OVERFLOW
    ...
    # T3 = fib(T1)
        	LDR R0 (SP)      # value in return address (904) 
	...
	# Set FP before jumping
        	...
        	STR R1 T2          # store in T2 variable for f(n-2)
	...
	# ___________________fib(T2)___________________________
   			MOV  R7  SP		 	# Test for overflow SP < SL
    ...
    # T4 = fib(T2)
	    	LDR R0 (SP)         # value in return address
